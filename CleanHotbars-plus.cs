...
namespace clean_hotbars
{
    ...
    public class CleanHotbars : BaseUnityPlugin
    {
        ...
        private static void ClearSmallNumbers(GameObject imgObj)
        {
            var imgComp = imgObj.GetComponent("UnityEngine.UI.Image");
            if (imgComp != null)
            {
                Destroy(imgComp);
            }

            Transform childTransform = imgObj.transform.Find("Text (TMP)");
        ...
        private static void ClearExtendedHotbars()
        {
            ...
            allTransforms = Resources.FindObjectsOfTypeAll<Transform>();
            foreach (var tr2 in allTransforms)
            {
                if (tr2.name == "Image" && tr2.parent != null && tr2.parent.name == "HK1(Clone)" && tr2.Find("Text (TMP)") != null)
                {
                    ClearSmallNumbers(tr2.gameObject);
                }
            }
        }
    }
